...
def generate_refsnak(source, url, date):
    ...
    return [[
        wdi_core.WDItemID(is_reference=True, value=source, prop_nr='P248'),
...
def get_pubmed(identifiers, dbname='pubmed'):
    ...
    for bundle in bundles:
        ...
        summary_retriever = requests.get(esummary_url +
                                         bunch_of_numbers).json()
...
def get_data(manifest):
    ...
    for entry in package:
        ...
        if 'pmcid' in entry['raw_data']:
            pubmed_data_source = ('pmc',
                                  'Q229883')  # for use in refsnak generator
        ...
        if pubmed_data != {}:
            ...
            pubmed_ref = generate_refsnak(pubmed_data_source[1],
...
def item_creator(manifest):
    ...
    for new_entry in retrieved_data:
        ...
        i = wdi_core.WDItemEngine(
            data=data, item_name=label, domain='journalarticles')
        i.set_label(label)
        try:
